...
func Compile(args []string, stmt stmt.Stmt, passPackageFunc func(pkgPath string) bool) (newArgs []string, err error) {
	...
	log.Logger.Info(flag.PkgPath)
	if passPackageFunc(flag.PkgPath) {
		return
	}
	log.Logger.Debug(flag.ImportCfgPath)
	i := instrumentation.NewDst(flag.PkgPath, flag.ImportCfgPath, stmt)
	...
	src2dst, err := i.WriteInstrumentedFiles(filepath.Dir(flag.Output))
	importcfg.Dump(flag.ImportCfgPath)
